            ...
            <div class="row">
                <div class="col-md-12 mt-3 mb-3">
                    <x-upload-wrapper :max-files="$tool->no_file_tool" :max-size="$tool->fs_tool"
                        accept=".png,.jpeg,.gif,.tif,.webp,.jfif,.nefp" input-name="images[]" :file-title="__('tools.dropImageHereTitle')"
                        :file-label="__('tools.convertToJpgDesc')" />
                    ...
                    const process_id = '{{ $results['process_id'] }}',
                        ...
                        startConversion = async function(files, cursor) {
                            ...
                            showDownload = function(data, index) {
                                const button =
                                    `<button class="btn btn-outline-primary download-file-btn" type="button" id="download-${index}" data-url="${data.url}" data-filename="${data.filename}">
                                            {{ __('common.download') }}
                                        </button>`
                                if (data.success) {
                                    processed += 1;
                                ...
                                document.getElementById('file-cursor-' + index).innerHTML = data.success ? button :
                                    '<span class="badge bg-danger rounded-pill">{{ __('common.failed') }}</span>'
