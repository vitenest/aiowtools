...
class SvgToPng implements ToolInterface
{
    public function render(Request $request, Tool $tool)
    {
        return view('tools.svg-to-png', compact('tool'));
    }
    ...
    public function handle(Request $request, Tool $tool)
    {
        ...
        return view('tools.svg-to-png', compact('results', 'tool'));
    }
    ...
    protected function processSingleFile($request)
    {
        ...
        // Ensure the image has transparency by keeping the alpha channel active
        $imagick->setImageAlphaChannel(\Imagick::ALPHACHANNEL_ACTIVATE);

        // Set the background color to 'none' (transparent)
        $imagick->setImageBackgroundColor(new \ImagickPixel('none'));

        // Flatten the image to ensure the transparent background is applied
