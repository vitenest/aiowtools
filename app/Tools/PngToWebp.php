...
class PngToWebp implements ToolInterface
{
    public function render(Request $request, Tool $tool)
    {
        return view('tools.png-to-webp', compact('tool'));
    }
    ...
    public function handle(Request $request, Tool $tool)
    {
        $validated = $request->validate([
            ...
            'images.*' => "image|mimes:jpg,png,jpeg|max:" . $tool->fs_tool * 1024
        ]);
        ...
        return view('tools.png-to-webp', compact('results', 'tool'));
    }
    ...
    protected function processSingleFile($request)
    {
        ...
        $result = tempFileUploadToImageConverter($file, 'webp', true, false, $process_id);
        Cache::put($process_id . "-download-all", $result, job_cache_time());

        return response()->json([
            'success' => true,
            'filename' => trim_extension($result['original_filename']) . '.webp',
            'size' => $result['size'],
            'url' => url($result['url']),
        ]);
    }
