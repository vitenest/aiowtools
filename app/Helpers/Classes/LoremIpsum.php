...
class LoremIpsum
{
     ...
     * @return mixed   string or array of generated lorem ipsum sentences
     */
    public function lists($count = 1, $tags = false, $array = false)
    {
        $sentences = array();
        ...
        for ($i = 0; $i < $count; $i++) {
            $number = rand(1, 5);
            for ($j = 0; $j < $number; $j++) {
                $sentences[$i][] = $this->wordsArray(rand(5, 13));
            }
        }
        ...
        foreach ($sentences as $index => $list) {
            ...
            $lists[] = $this->output($list, $tags, $array);
        }

        return $this->output($lists, 'ul', $array);
    }
